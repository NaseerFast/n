...
import { ChevronDownIcon, ChevronUpIcon, FilterIcon, MinusSmIcon, PlusSmIcon, ViewGridIcon, CheckIcon } from '@heroicons/react/solid'
...
// Add/Remove checked item from list
  ...
  const router = useRouter();
   ...
   const {
    ...
    const { joblistings, countProducts, categories, locations, cities, allratings, pages } = props;
  ...
  {({ open }) => (
    ...
    {categories && categories.map((item, index) => (
        ...
        {/* <input className="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-neutral-800 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox"  value={item} id="topping" onChange={handleCategoryCheck}
         /> */}
          <NextLink href={`/jobs/search?category=${item}`} passHref  >
         <li className=" text-gray-300 font-medium hover:underline hover:text-yellow-300 cursor-pointer" htmlFor="" key={index}>
          {item}
         </li>
         </NextLink>
       </div>
...
<div className=" lg:mr- lg:mb-0 mb-7 p-6 bg-neutral-800  shadow rounded">
                    <div className="flex items-center border-b border-gray-200 pb-6">
                        ...
                        <div className="flex items-start justify-between w-full">
                            <div className="pl-3 w-full">
                                <p className="text-sm font-medium leading-5 text-gray-400">{item.title}</p>
                                <p className="text-xs leading-normal pt-2 text-gray-500">{item.employer + ", " + (`${item.location}`)}</p>
                            </div>
  ...
  export async function getServerSideProps({ query }) {
    ...
    const categories = await JobListing.find().distinct('category');
    const locations = await JobListing.find().distinct('location');
    const cities = await JobListing.find().distinct('city');
    const allratings = await JobListing.find().distinct('rating');
    const joblistingDocs = await JobListing.find(
      {
        ...queryFilter,
        ...categoryFilter,
        ...priceFilter,
    ...
    const countProducts = await JobListing.countDocuments({
      ...queryFilter,
      ...categoryFilter,
      ...priceFilter,
      ...locationFilter,
    ...
    const joblistings = joblistingDocs.map(db.convertDocToObj);
